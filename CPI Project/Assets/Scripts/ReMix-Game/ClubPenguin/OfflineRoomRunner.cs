...
namespace ClubPenguin
{
    public class OfflineRoomRunner : IOfflineRoomRunner
    {
        ...
        private class TubeRaceWorldObject : IWorldObject
        {
            ...
            {
                if (!usersInRace.Contains(action.SessionId))
                {
            ...
            internal void StartRace()
            {
                ...
                {
                    List<long> list = new List<long>(usersInRace);
                    usersInRace.Clear();
                    cpItem.playerData = "{\"Players\":[]}";
                    processEvent(GameServerEvent.SERVER_ITEM_CHANGED, cpItem);
                    foreach (long item in list)
                    {
                        processEvent(GameServerEvent.HELD_OBJECT_DEQUIPPED, item);
                    }
                    TubeRaceGameSession partyGame = new TubeRaceGameSession(list, cpItem, processEvent, partyGameSessionManager, raceDefinition);
                    partyGameSessionManager.StartSession(partyGame);
                }
            }

            internal void RemoveUser(long sessionId)
            {
                if (usersInRace.Remove(sessionId))
                {
                    cpItem.playerData = getPlayerData(usersInRace);
                    processEvent(GameServerEvent.SERVER_ITEM_CHANGED, cpItem);
                    PlayerLocomotionStateEvent playerLocomotionStateEvent = default(PlayerLocomotionStateEvent);
                    playerLocomotionStateEvent.SessionId = sessionId;
        ...
        private class TubeRaceGameSession : OfflineGameServerClient.IPartyGameSession
        {
            ...
            {
                TubeRaceScoreModifier[] array = UnityEngine.Object.FindObjectsByType<TubeRaceScoreModifier>(FindObjectsSortMode.None);
                int num = 0;
        ...
        public void Start()
        {
            ...
            eventChannel.AddListener<PlayerStateServiceEvents.HeldObjectDequipped>(onHeldObjectDequipped);
        }
        ...
        private bool onHeldObjectDequipped(PlayerStateServiceEvents.HeldObjectDequipped evt)
        {
            foreach (IWorldObject worldObject in worldObjects)
            {
                TubeRaceWorldObject tubeRaceWorldObject = worldObject as TubeRaceWorldObject;
                if (tubeRaceWorldObject != null)
                {
                    tubeRaceWorldObject.RemoveUser(evt.SessionId);
                }
            }
            return false;
        }
        ...
        {
            ScheduledWorldObjectConfiguration[] array = UnityEngine.Object.FindObjectsByType<ScheduledWorldObjectConfiguration>(FindObjectsSortMode.None);
        ...
        private void startTubeRacingSchedule()
        {
            ...
            TubeRaceLobby[] array = UnityEngine.Object.FindObjectsByType<TubeRaceLobby>(FindObjectsSortMode.None);
